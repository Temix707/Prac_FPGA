...
int main() {
    ...
    while(1) {
        ...
        for (int i=0; i<KUZNECHIK_BLOCK_LEN/2; i++) {
            uint8_t t = src_data[i];
            src_data[i] = src_data[KUZNECHIK_BLOCK_LEN-1-i];
            src_data[KUZNECHIK_BLOCK_LEN-1-i] = t;
        }

        if (kuznechik_process_data_block(src_data, dst_data)) {

            for (int i=0; i<KUZNECHIK_BLOCK_LEN/2; i++) {
                uint8_t t = dst_data[i];
                dst_data[i] = dst_data[KUZNECHIK_BLOCK_LEN-1-i];
                dst_data[KUZNECHIK_BLOCK_LEN-1-i] = t;
            }
