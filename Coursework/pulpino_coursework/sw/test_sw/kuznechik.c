...
void kuznechik_init() {
    CGREG |= (1 << CGKUZ);                                       // Enabling clocking

    KUZNECHIK = (struct KUZNECHIK_REGS*) KUZNECHIK_BASE_ADDR;
    KUZNECHIK->RST = 1;                                          // Resetting the device
}
...
bool kuznechik_process_data_block( const void *src, void *dst ) {
    ...
    // we can't wait for the module to be released indefinitely
    const int max_wait_cycles = 16000;

    // we waiting for the release of the encryption module
    ...
    // read the encrypted data from the device register
